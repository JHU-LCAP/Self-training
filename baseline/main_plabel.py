...
def train(train_loader, model, optimizer, c_epoch, ema_model=None, mask_weak=None, mask_strong=None, adjust_lr=False):
    ...
    class_criterion, consistency_criterion, softmax = to_cuda_if_available(class_criterion, consistency_criterion, softmax)
    ...
    #plabel = []
    for i, ((batch_input, _), target) in enumerate(train_loader):
        ...
        strong_predict, weak_predict= ema_model(batch_input)
        ...
        reliability = rampup_weight/class_criterion(target[mask_strong], est_strong_target[mask_strong]).mean()
        reliability = torch.clamp(reliability, 0, 5)
        meters.update('Reliability of pseudo label', reliability.item())

        # classification error with pseudo label
        pred_strong_loss = consistency_criterion(strong_pred[:n_unlabeled], est_strong_target[:n_unlabeled]).mean([1,2])
        pred_weak_loss  = consistency_criterion(weak_pred[:n_unlabeled], est_weak_target[:n_unlabeled]).mean(1)
        ...
        loss = weak_class_loss + strong_class_loss + expect_loss
...
if __name__ == '__main__':
    ...
    store_dir = os.path.join("stored_data", "MeanTeacher_with_plabel")
